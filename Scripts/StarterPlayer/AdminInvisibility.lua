--[[
  Type:     ModuleScript
  Method:   decompile
  Name:     AdminInvisibility
  Path:     game.StarterPlayer.StarterPlayerScripts.Client.Components.AdminInvisibility
...
return function() -- Line: 18
    ...
    local function hideBillboard(p2: any, u3: userdata) -- Line: 30
        ...
        u3.Enabled = false;
        p2.holds[u3] = u3:GetPropertyChangedSignal("Enabled"):Connect(function() -- Line: 37
            -- upvalues: u3 (copy)
            if u3.Enabled then
                u3.Enabled = false;
            end;
    ...
    local function clearBillboards(p4) -- Line: 45
    ...
    local function bindCharacter(u5: any, p6: userdata) -- Line: 53
        ...
        u5.descendantConn = p6.DescendantAdded:Connect(function(p7) -- Line: 62
            -- upvalues: hideBillboard (ref), u5 (copy)
            hideBillboard(u5, p7);
        end);
    end;

    local function activate(p8: userdata) -- Line: 67
        ...
        u10.characterConn = p8.CharacterAdded:Connect(function(p11) -- Line: 79
    ...
    local function deactivate(p12: userdata) -- Line: 86
        -- upvalues: u1 (copy)
        local v13 = u1[p12];
    ...
    local function apply(p14: userdata) -- Line: 111
        -- upvalues: activate (copy), deactivate (copy)
        if p14:GetAttribute("AdminInvisible") == true then
            activate(p14);
    ...
    local function watchPlayer(u15: userdata) -- Line: 119
        -- upvalues: activate (copy), deactivate (copy)
        if u15:GetAttribute("AdminInvisible") == true then
            activate(u15);
        else
            deactivate(u15);
        end;

        u15:GetAttributeChangedSignal("AdminInvisible"):Connect(function() -- Line: 121
            -- upvalues: u15 (copy), activate (ref), deactivate (ref)
            local v16 = u15;

            if v16:GetAttribute("AdminInvisible") == true then
                activate(v16);
    ...
    for _, v in Players:GetPlayers() do
        if v:GetAttribute("AdminInvisible") == true then
            activate(v);
        else
            deactivate(v);
        end;

        v:GetAttributeChangedSignal("AdminInvisible"):Connect(function() -- Line: 121
            -- upvalues: v (copy), activate (copy), deactivate (copy)
            local v17 = v;

            if v17:GetAttribute("AdminInvisible") == true then
                activate(v17);
    ...
    Players.PlayerRemoving:Connect(function(p18) -- Line: 131
        -- upvalues: u1 (copy)
        local v19 = u1[p18];
