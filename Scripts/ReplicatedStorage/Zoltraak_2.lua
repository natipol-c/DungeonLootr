--[[
  ...
  Method:   decompile
  Name:     Zoltraak
  Path:     game.ReplicatedStorage.Classes.Demonbane.Skills.Zoltraak
...
local u1 = {
    ...
    HitboxSize = Vector3.new(20, 15, 35),
    HitboxRange = 35,
    BlastFX = "Ability_1",
    BlastSFX = "Sieghart_Spell_15",
    SFXVolume = 1,
    MaxDuration = 1.5
};
...
function u1.CanActivate(p10) -- Line: 95
...
function u1.Activate(u11, p12) -- Line: 103
    -- upvalues: u1 (copy), SharedUtils (copy)
    local v13 = u1._EnsureAnimation(u11);

    if not v13 then
        warn("[Zoltraak] Animation not found");
    ...
    local function disconnectAll() -- Line: 126
        -- upvalues: u14 (copy)
        for _, v in u14 do
            v:Disconnect();
        end;
    ...
    task.delay(u1.MaxDuration, function() -- Line: 161
        -- upvalues: u15 (ref), u11 (copy), u14 (copy)
        if not u15 then
            u15 = true;
            u11.Is_Using_Skill = false;
