--[[
  Type:     ModuleScript
  Method:   decompile
  Name:     KillerCadence_Sword
  Path:     game.ReplicatedStorage.GameInfo.Boss_Abilities.Unrestricted EX.KillerCadence_Sword
...
local function forwardVFX(u4, p5, p6) -- Line: 43
    -- upvalues: u1 (copy)
    local function emit(p7) -- Line: 44
...
function u1._PerformHit(p9, p10) -- Line: 54
    -- upvalues: u1 (copy)
...
function u1.Activate(u13, u14) -- Line: 62
    -- upvalues: u1 (copy), forwardVFX (copy), SharedUtils (copy)
    local v15 = u13.Animations[u14._animKey or u1.AnimationName];

    if not v15 then
        warn("[Boss KillerCadence_Sword] Animation not found:", u14._animKey or u1.AnimationName);
    ...
    table.insert(v17, MarkerReachedSignal:Connect(function() -- Line: 83
        -- upvalues: u13 (copy), SharedUtils (ref), u14 (copy), u1 (ref)
        local v18 = u13.Character and u13.Character:FindFirstChild("HumanoidRootPart");

        if v18 then
            SharedUtils.PlaySoundAt(v18, "claw4", 1);
        end;
