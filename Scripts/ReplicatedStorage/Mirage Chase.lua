--[[
  ...
  Method:   decompile
  Name:     Mirage Chase
  Path:     game.ReplicatedStorage.Classes.Awakened Devil EX.Skills.Mirage Chase
...
function u2._EnsureAnimation(p3) -- Line: 74
...
function u2._PerformHit(p8) -- Line: 101
    -- upvalues: u2 (copy)
...
function u2._Dash(p11, p12) -- Line: 127
...
function u2._AirDash(p14, u15, p16) -- Line: 157
    -- upvalues: u2 (copy)
    ...
    task.delay(u2.DashDuration, function() -- Line: 168
        -- upvalues: u15 (copy)
...
function u2._SpawnClones(u18) -- Line: 176
...
function u2.CanActivate(p20) -- Line: 198
...
function u2.Activate(u21, p22) -- Line: 206
    -- upvalues: u2 (copy), SharedUtils (copy)
    local v23 = u2._EnsureAnimation(u21);

    if not v23 then
        warn("[Mirage Chase] Animation not found");
    ...
    task.delay(u2.IFrameDuration, function() -- Line: 231
        -- upvalues: u21 (copy)
    ...
    local function disconnectAll() -- Line: 259
        -- upvalues: u26 (copy)
        for _, v in u26 do
            v:Disconnect();
        end;
    ...
    local function releaseState() -- Line: 269
        ...
        local ClassData = u21.ClassData;

        if ClassData and ClassData.OnSwingEnd then
            ClassData.OnSwingEnd(u21, nil);
        end;
    ...
    local function emitVFX(p29) -- Line: 301
        -- upvalues: u21 (copy), u2 (ref)
        if not p29 or p29 == "" then
            return;
        end;

        local v30 = u21.Character and u21.Character:FindFirstChild("HumanoidRootPart");

        if not v30 then
            return;
        end;

        u21:PlayEffectModule(u2.EffectModule, "Emit", v30.CFrame, p29);
    end;

    u26[#u26 + 1] = v23:GetMarkerReachedSignal("VFX"):Connect(emitVFX);
    u26[#u26 + 1] = v23:GetMarkerReachedSignal("VFX_2"):Connect(emitVFX);
    u26[#u26 + 1] = v23:GetMarkerReachedSignal("hit"):Connect(function(p31) -- Line: 311
        -- upvalues: u21 (copy), u2 (ref)
        u21:PlayCombatSound(u21.ClassData.SwingSoundFolder or "Power_Swing_Fast", nil, u21.ClassData.SwingVolume or 0.5);
        u21:ShakeCamera("Hit");
        u2._PerformHit(u21);
    end);
    u26[#u26 + 1] = v23:GetMarkerReachedSignal("sheathe"):Connect(function() -- Line: 319
        -- upvalues: u21 (copy), SharedUtils (ref), u2 (ref)
        local v32 = u21.Character and u21.Character:FindFirstChild("HumanoidRootPart");

        if v32 then
            SharedUtils.PlaySoundAt(v32, "Sheathe_1", u2.SheatheVolume);
        end;
    end);
    u26[#u26 + 1] = v23:GetMarkerReachedSignal("DBreset"):Connect(releaseState);
    v23.Stopped:Once(function() -- Line: 332
    ...
    task.delay(u2.MaxDuration, function() -- Line: 339
        -- upvalues: u27 (ref), u25 (ref), u21 (copy), u26 (copy)
        if not u27 then
            u27 = true;
