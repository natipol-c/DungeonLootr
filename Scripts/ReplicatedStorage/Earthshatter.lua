--[[
  ...
  Method:   decompile
  Name:     Earthshatter
  Path:     game.ReplicatedStorage.GameInfo.Boss_Abilities.Framebreaker.Earthshatter
...
local u1 = {
    ...
    MaxDuration = 4.5,
    HitboxSize = Vector3.new(28, 22, 32),
    HitboxRange = 28
};
...
function u1.Activate(u6, u7) -- Line: 52
    -- upvalues: u1 (copy)
    local v8 = u6.Animations[u1.AnimationName];

    if not v8 then
        warn("[Boss Earthshatter] Animation not found:", u1.AnimationName);
    ...
    local v11 = v8:GetMarkerReachedSignal("hit"):Connect(function(p10) -- Line: 74
        -- upvalues: u7 (copy), u6 (copy), u1 (ref)
        u6:PlayCombatSound(u7.SwingSoundFolder or (u6.ClassData.SwingSoundFolder or "Naoya_Punches"), nil, u6.ClassData.SwingVolume or 1);
    ...
    v8.Stopped:Once(function() -- Line: 85
        -- upvalues: u9 (ref)
        u9 = true;
    end);
    task.delay(u1.MaxDuration, function() -- Line: 90
        -- upvalues: u9 (ref)
        u9 = true;
    end);
