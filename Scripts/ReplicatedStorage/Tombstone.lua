--[[
  ...
  Method:   decompile
  Name:     Tombstone
  Path:     game.ReplicatedStorage.GameInfo.Boss_Abilities.Reaper.Tombstone
...
local u1 = {
    ...
    MaxDuration = 3,
    HitboxSize = Vector3.new(15, 10, 22),
    HitboxRange = 22,
    HitboxRange = 22
};
...
function u1.Activate(u6, u7) -- Line: 65
    ...
    if not v8 then
        warn("[Boss Tombstone] Animation not found:", u1.AnimationName);
    ...
    local v13 = v8:GetMarkerReachedSignal("dash"):Connect(function(p10) -- Line: 87
        ...
        SharedUtils.PlaySoundAt(v11, "Dark_Dash_2", 1);
        local v12;
    ...
    local u14 = 0;
    local v17 = v8:GetMarkerReachedSignal("hit"):Connect(function(p15) -- Line: 109
        -- upvalues: u14 (ref), u6 (copy), SharedUtils (ref), u1 (ref), u7 (copy)
        u14 = u14 + 1;
        local v16 = u6.Character and u6.Character:FindFirstChild("HumanoidRootPart");

        if v16 then
            if u14 == 1 then
                SharedUtils.PlaySoundAt(v16, "Dark_Swing", 1);
            else
                SharedUtils.PlaySoundAt(v16, "Dark_Echo", 1);
            end;

            SharedUtils.PlaySoundAt(v16, "Rolling_Swing", 1);
        end;
    ...
    v8.Stopped:Once(function() -- Line: 129
        -- upvalues: u9 (ref)
        u9 = true;
    end);
