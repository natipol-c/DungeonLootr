--[[
  ...
  Method:   decompile
  Name:     Dodge_Blossom
  Path:     game.ReplicatedStorage.Classes.Ronin.Mastery_Passives.Dodge_Blossom
...
local v4 = {
    ...
    Init = function(p1) -- Line: 43, Name: Init
        ...
        table.insert(p1.MasteryPassives.OnDodge, {
            Name = "Dodge_Blossom_Flag",
            Cooldown = 0,

            Execute = function(p2, p3) -- Line: 51, Name: Execute
                p2._dodgeBlossomReady = tick();
            end
        });
    end
};
...
local function _EnsureAnimation(p5) -- Line: 57
    ...
    if p5.Animations.DodgeBlossom_Ability_4_Passive then
        return p5.Animations.DodgeBlossom_Ability_4_Passive;
    end;
    ...
    v8:Play(0, 0, 0);
    v8:Stop(0);
    p5.Animations.DodgeBlossom_Ability_4_Passive = v8;
...
function v4.Execute(u12, p13) -- Line: 104
    ...
    local _dodgeBlossomReady = u12._dodgeBlossomReady;

    if not _dodgeBlossomReady then
        return;
    end;

    if tick() - _dodgeBlossomReady > 3 then
        u12._dodgeBlossomReady = nil;
