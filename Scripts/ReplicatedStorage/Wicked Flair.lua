--[[
  ...
  Method:   decompile
  Name:     Wicked Flair
  Path:     game.ReplicatedStorage.Weapons.Weapon_Skills.Wicked Flair
...
u1.MaxCharges = 4;
u1.ChargeCooldown = 0.1;
u1.ChargeExpiry = 5;
u1.SlideDuration = 0.29;
u1.SlideVelocity = 78;
u1.IFrameDuration = 0.35;
u1.DamageMultiplier = 1;
u1.HitboxSize = Vector3.new(18, 25, 19);
u1.CloneFadeDuration = 0.5;
u1.StartSound = "WickedFlair_Start";
u1.SlashSound = "WickedFlair_Shot";
u1.EndSound = "WickedFlair_End";
...
local function PlayTurnFX(p3, p4) -- Line: 73
    ...
    if not v5 then
        warn("WickedFlair: Missing FX part: " .. tostring(p4));
...
function u1.CanActivate(p14) -- Line: 112
    ...
    if p14._WickedFlair_Charges and p14._WickedFlair_Charges > 0 then
        if p14._WickedFlair_AnimPlaying then
...
function u1.Activate(u15) -- Line: 135
    ...
    local v18 = not u15._WickedFlair_Charges or u15._WickedFlair_Charges == 0;
    Character:SetAttribute("Skill_Attack_Bypass", true);

    if v18 then
        u15._WickedFlair_Charges = u1.MaxCharges;
        u15._WickedFlair_ExpiryToken = tick();
    ...
    u15._WickedFlair_Charges = u15._WickedFlair_Charges - 1;
    local _WickedFlair_Charges = u15._WickedFlair_Charges;
    Player:SetAttribute("Skill_Charges", _WickedFlair_Charges);
    u1._ExecuteCharge(u15, u16, v17, _WickedFlair_Charges);

    if _WickedFlair_Charges <= 0 then
    ...
    task.delay(u1.ChargeCooldown, function() -- Line: 181
        ...
        if u15._WickedFlair_Charges and (u15._WickedFlair_Charges > 0 and Player) then
            Player:SetAttribute("Skill_OnCooldown", false);
        end;
    end);
    local u19 = tick();
    u15._WickedFlair_ExpiryToken = u19;
    task.delay(u1.ChargeExpiry, function() -- Line: 193
        -- upvalues: u15 (copy), u19 (copy), u1 (ref), Skill (copy), Character (copy)
        if u15._WickedFlair_ExpiryToken ~= u19 then
            return;
        end;

        if not u15._WickedFlair_Charges or u15._WickedFlair_Charges <= 0 then
            return;
        end;
...
function u1._EnsureAnimations(p20) -- Line: 214
    ...
    if not v21 then
        warn("WickedFlair: No Animator found");

        return;
    end;

    local v22 = ReplicatedStorage.Weapons:FindFirstChild("Wicked Fair");

    if not v22 then
        warn("WickedFlair: Wicked Fair weapon folder not found");
    ...
    if not Animations then
        warn("WickedFlair: Animations folder not found");

        return;
    end;

    if not p20.Animations.WickedFlair_Ability then
        ...
        if Ability then
            ...
            p20.Animations.WickedFlair_Ability = v23;
        else
            warn("WickedFlair: Ability animation not found in Lucky Revolvers");
        end;
    end;

    if not p20.Animations.WickedFlair_Dodge then
        local Dodge = Animations:FindFirstChild("Dodge");

        if Dodge then
            local v24 = v21:LoadAnimation(Dodge);
            v24.Priority = Enum.AnimationPriority.Action2;
            p20.Animations.WickedFlair_Dodge = v24;

            return;
        end;

        warn("WickedFlair: Dodge animation not found in Lucky Revolvers");
    end;
end;
...
function u1._ExecuteCharge(u25, u26, p27, u28) -- Line: 265
    ...
    u25._WickedFlair_AnimPlaying = true;
    ...
    local WickedFlair_Ability = u25.Animations.WickedFlair_Ability;

    if not WickedFlair_Ability then
        warn("WickedFlair: Ability animation not loaded");
        u25.Is_Using_Skill = false;
        u25.Is_Attacking = false;
        u25._WickedFlair_AnimPlaying = false;

        return;
    end;

    WickedFlair_Ability:Play(0.3, 1, 1.3);
    local u32 = WickedFlair_Ability:GetMarkerReachedSignal("hit"):Connect(function(p30) -- Line: 312
        ...
        if p30 and p30 ~= "" then
            ...
            if v31 then
                ...
                warn("WickedFlair: Missing FX part: " .. tostring(p30));
            end;
        end;
    ...
    BodyVelocity.Name = "WickedFlairVelocity";
    BodyVelocity.MaxForce = Vector3.new(100000, 0, 100000);
    BodyVelocity.Velocity = v29 * u1.SlideVelocity;
    BodyVelocity.Parent = u26;
    Debris:AddItem(BodyVelocity, u1.SlideDuration);
    WickedFlair_Ability.Stopped:Once(function() -- Line: 353
        -- upvalues: u32 (ref), u25 (copy)
        if u32 then
            u32:Disconnect();
            u32 = nil;
        end;

        if dodgeAnim and dodgeAnim.IsPlaying then
            dodgeAnim:Stop(0.15);
        end;
        ...
        if not u25._WickedFlair_Charges or u25._WickedFlair_Charges <= 0 then
            u25.Is_Using_Skill = false;
        end;
    end);
    task.delay(u1.IFrameDuration, function() -- Line: 373
    ...
    task.delay((WickedFlair_Ability.Length > 0 and WickedFlair_Ability.Length or 1) + 0.5, function() -- Line: 384
        -- upvalues: u32 (ref), u25 (copy)
        if u32 then
            u32:Disconnect();
            u32 = nil;
...
function u1._PerformHit(p33) -- Line: 400
...
function u1._ForceExpire(p38, p39) -- Line: 482
    -- upvalues: u1 (copy)
    p38._WickedFlair_Charges = 0;
    p38._WickedFlair_ExpiryToken = nil;
    p38._WickedFlair_AnimPlaying = false;
...
function u1._StartFullCooldown(p40, p41) -- Line: 500
    p40._WickedFlair_Charges = 0;
    p40._WickedFlair_ExpiryToken = nil;
